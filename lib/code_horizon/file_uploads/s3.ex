defmodule CodeHorizon.FileUploads.S3 do
  @moduledoc """
  ...
  In your live view mount() function:
      ...
      |> allow_upload(:avatar,
          ...
          max_entries: 1,
          external: &CodeHorizon.FileUploads.S3.presign_upload/2
        )}

  When you want to retrieve the URLs:

      def handle_event("submit", %{"user" => user_params}, socket) do
        uploaded_files = CodeHorizon.FileUploads.S3.consume_uploaded_entries(socket, :avatar)
  ...
  def presign_upload(entry, socket) do
    ...
    scheme = CodeHorizon.config([:s3, :scheme])
    bucket = CodeHorizon.config([:s3, :bucket])
    key = Ecto.UUID.generate() <> Path.extname(entry.client_name)

    config = %{
      region: CodeHorizon.config([:s3, :region]),
      access_key_id: CodeHorizon.config([:s3, :access_key]),
      secret_access_key: CodeHorizon.config([:s3, :secret])
    }
