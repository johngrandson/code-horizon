defmodule CodeHorizon.Logs do
  ...
  def create(attrs \\ %{}) do
    case %Log{}
         ...
         |> Repo.insert() do
      {:ok, log} ->
        CodeHorizonWeb.Endpoint.broadcast("logs", "new-log", log)
        {:ok, log}
  ...
  def log_async(action, params) do
    CodeHorizon.BackgroundTask.run(fn ->
      log(action, params)
    end)
  end
  ...
  Examples:
      ...
      CodeHorizon.Logs.log("orgs.create_invitation", %{
      ...
      CodeHorizon.Logs.log("orgs.delete_member", %{
        user: socket.assigns.current_user,
  ...
  def exists?(params) do
    ...
    |> CodeHorizon.Repo.exists?()
  end

  def get_last_log_of_user(user) do
    user.id
    |> CodeHorizon.Logs.LogQuery.by_user()
