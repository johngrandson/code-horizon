defmodule CodeHorizonWeb.EditTotpLive do
  @moduledoc false
  ...
  def handle_event("regenerate_backup_codes", _, socket) do
    ...
    CodeHorizon.Logs.log_async("totp.regenerate_backup_codes", %{user: socket.assigns.current_user})
    {:noreply, socket}
  end
  ...
  def handle_event("update_totp", %{"user_totp" => params}, socket) do
    ...
    case Accounts.upsert_user_totp(editing_totp, params) do
      {:ok, current_totp} ->
        CodeHorizon.Logs.log_async(log_type, %{user: socket.assigns.current_user})
  ...
  def handle_event("disable_totp", _, socket) do
    ...
    CodeHorizon.Logs.log_async("totp.disable", %{user: socket.assigns.current_user})
  ...
  def handle_event("submit_totp", %{"user" => %{"current_password" => current_password}}, socket) do
    ...
    if socket.assigns.user_form.source.valid? do
      ...
      app = CodeHorizon.config(:app_name)
  ...
  defp generate_qrcode(uri) do
    if CodeHorizon.config(:env) == :test do
      raw("")
    else
      uri
      |> EQRCode.encode()
