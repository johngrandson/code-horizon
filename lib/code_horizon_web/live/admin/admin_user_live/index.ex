defmodule CodeHorizonWeb.AdminUserLive.Index do
  ...
  alias CodeHorizonWeb.DataTable
  alias CodeHorizonWeb.UserAuth

  @billing_provider Application.compile_env(:code_horizon, :billing_provider)
  ...
  def mount(_params, _session, socket) do
    if connected?(socket),
      do: Phoenix.PubSub.subscribe(CodeHorizon.PubSub, "users")
  ...
  def apply_action(socket, :index, params) do
    ...
    |> assign(
      ...
      changeset: nil,
      form: nil,
      online_users: "users" |> CodeHorizonWeb.Presence.list() |> Map.keys() |> Enum.map(&String.to_integer/1),
      page_title: gettext("Users")
    )
  end
    ...
    Enum.reduce(joins, socket, fn {user_id, _}, socket ->
      users =
        Enum.map(socket.assigns.users, fn user ->
          if Integer.to_string(user.id) == user_id, do: CodeHorizonWeb.Presence.online_user?(user), else: user
        end)
  ...
  defp assign_users(socket, params) do
    ...
    users = CodeHorizonWeb.Presence.online_users(users)
  ...
  def user_actions(assigns) do
    ...
    <div class="flex items-center" id={"user_actions_container_#{@user.id}"}>
      ...
      >
        ...
        <%= if CodeHorizon.Accounts.Permissions.can_impersonate?(@user, @current_user) do %>
