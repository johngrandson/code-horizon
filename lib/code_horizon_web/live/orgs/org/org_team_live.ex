defmodule CodeHorizonWeb.OrgTeamLive do
  @moduledoc false
      ...
      {:ok, membership} ->
        ...
        if membership.user_id == socket.assigns.current_user.id do
          {:noreply,
           ...
           |> push_navigate(to: CodeHorizonWeb.Helpers.home_path(socket.assigns.current_user))}
        else
          CodeHorizonWeb.UserAuth.disconnect_user_liveviews(member_user)
  ...
  def handle_event("delete_invitation", %{"id" => id}, socket) do
    ...
    CodeHorizon.Logs.log("orgs.delete_invitation", %{
      user: socket.assigns.current_user,
      org_id: org.id,
      target_user_id: nil,
      metadata: %{
