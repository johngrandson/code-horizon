defmodule CodeHorizonWeb.AdminAiChatLive.LangChain do
  @moduledoc """
        ...
        parameters: [
          FunctionParam.new!(%{
            ...
            **This must be a string that can be passed to Code.eval_string/1 using Elixir code that queries the database. It cannot be plain SQL.**
            ...
            Our Ecto Repository is **CodeHorizon.Repo** and you can use the Ecto.Query.from/1 function call, e.g. `Ecto.Query.from(p in \"users\", select: p) |> CodeHorizon.Repo.all()`.
            Instead of using CodeHorizon.Repo.all(), you can also use CodeHorizon.Repo.one() depending on if the result is one or many rows.
  ...
  def assistant(live_view_pid) do
    ...
    |> LLMChain.add_message(
      ...
      **If you want to list records, always use a syntax like `Ecto.Query.from(p in CodeHorizon.Accounts.User, where: p.foo == ^"bar") |> CodeHorizon.Repo.all()`.
  ...
  defp get_ecto_models do
    {:ok, modules} = :application.get_key(:code_horizon, :modules)
  ...
  defp get_ecto_contexts do
    ...
    modules
    |> Enum.filter(&Regex.match?(~r/^Elixir.CodeHorizon.\w+$/, "#{&1}"))
    |> Enum.map_join("\n\n", fn module ->
      functions =
        :functions
        |> module.__info__()
  ...
  defp get_gdpr_message do
    if CodeHorizon.config(:gdpr_mode),
      do: """
