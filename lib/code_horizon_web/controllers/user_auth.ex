defmodule CodeHorizonWeb.UserAuth do
  @moduledoc """
  ...
  alias CodeHorizon.Accounts.Permissions
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_code_horizon_web_user_remember_me"
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      CodeHorizonWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end

    if conn.assigns[:current_user] do
      CodeHorizon.Logs.log_async("sign_out", %{user: conn.assigns.current_user})
    end
  ...
  defp disconnect_user_tokens(users_tokens, delete_too? \\ false) do
    for user_token <- users_tokens do
      CodeHorizonWeb.Endpoint.broadcast(user_session_topic(user_token.token), "disconnect", %{})
  ...
  defp signed_in_path(current_user), do: CodeHorizonWeb.Helpers.home_path(current_user)
  ...
  def maybe_redirect_to_org_invitations(current_user) do
    invitations = CodeHorizon.Orgs.list_invitations_by_user(current_user)
