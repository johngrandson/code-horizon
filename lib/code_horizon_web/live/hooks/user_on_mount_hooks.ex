defmodule CodeHorizonWeb.UserOnMountHooks do
  @moduledoc """
  ...
  def on_mount(:require_authenticated_user, _params, session, socket) do
    ...
    if socket.assigns.current_user do
      CodeHorizonWeb.Presence.track(self(), "users", socket.assigns.current_user.id, %{})
      {:cont, socket}
    else
  ...
  defp maybe_subscribe_to_user_topics(%{assigns: %{current_user: %User{} = user}} = socket) do
    CodeHorizonWeb.Endpoint.subscribe(Notifications.user_notifications_topic(user.id))
    socket
  end
