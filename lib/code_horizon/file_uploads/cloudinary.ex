defmodule CodeHorizon.FileUploads.Cloudinary do
  @moduledoc """
  ...
  def update(assigns, socket) do
    {:ok,
     ...
     |> allow_upload(:image,
       accept: ~w(image/*),
       max_entries: 3,
       external: &CodeHorizon.FileUploads.Cloudinary.presign_upload/2
     )}
  ```
  ...
  defp file_id_to_url(public_id) do
    cloud_name = CodeHorizon.config([:cloudinary, :cloud_name])
    folder = CodeHorizon.config([:cloudinary, :folder])
  ...
  defp sign(config) do
    ...
    secret = CodeHorizon.config([:cloudinary, :api_secret])
    api_key = CodeHorizon.config([:cloudinary, :api_key])
  ...
  defp maybe_put_folder(config) do
    case CodeHorizon.config([:cloudinary, :folder]) do
      nil -> config
  ...
  @spec url :: String.t()
  defp url,
    do: Path.join(["https://api.cloudinary.com/v1_1", CodeHorizon.config([:cloudinary, :cloud_name]), "image/upload"])
end
