defmodule CodeHorizon.Email do
  @moduledoc """
  ...
  An Swoosh email struct can be delivered by a Swoosh mailer (see mailer.ex & user_notifier.ex). Eg:
      ...
      CodeHorizon.Email.confirm_register_email(user.email, url)
      |> CodeHorizon.Mailer.deliver()
  """

  use Phoenix.Swoosh,
    view: CodeHorizonWeb.EmailView,
    layout: {CodeHorizonWeb.EmailView, :email_layout}
  ...
  defp from_name do
    CodeHorizon.config(:mailer_default_from_name)
  end

  defp from_email do
    CodeHorizon.config(:mailer_default_from_email)
  end
  ...
  def get_unsubscribe_url(user, email_kind) do
    CodeHorizon.Accounts.NotificationSubscriptions.unsubscribe_url(user, email_kind)
  end
end
