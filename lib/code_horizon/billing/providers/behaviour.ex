defmodule CodeHorizon.Billing.Providers.Behaviour do
  @moduledoc """
  ...
  defmacro __using__(_) do
    quote do
      @behaviour CodeHorizon.Billing.Providers.Behaviour

      import CodeHorizon.Billing.Providers.Behaviour.UrlHelpers
    end
  end
  ...
  defmodule UrlHelpers do
    ...
    def success_url(:user, _user_id, customer_id) do
      url(CodeHorizonWeb.Endpoint, ~p"/app/subscribe/success?customer_id=#{customer_id}")
    end

    def success_url(:org, org_id, customer_id) do
      org = CodeHorizon.Orgs.get_org_by_id(org_id)
    ...
    def cancel_url(:user, _user_id), do: url(CodeHorizonWeb.Endpoint, ~p"/app/subscribe")

    def cancel_url(:org, org_id) do
      org = CodeHorizon.Orgs.get_org_by_id(org_id)

      url(CodeHorizonWeb.Endpoint, ~p"/app/org/#{org.slug}/subscribe")
    end
  end
end
