defmodule CodeHorizon.Billing.Providers.Stripe.Services.CreatePortalSessionTest do
  ...
  test "call/3 for user as source", %{user: user} do
    Application.put_env(:code_horizon, :billing_entity, :user)
    ...
    use_cassette "CodeHorizon.Billing.Providers.Stripe.Services.CreatePortalSession.call user" do
  ...
  test "call/3 for org as source", %{org: org} do
    Application.put_env(:code_horizon, :billing_entity, :org)
    ...
    use_cassette "CodeHorizon.Billing.Providers.Stripe.Services.CreatePortalSession.call org" do
      assert {:ok, %{url: "https://billing.stripe.com/p/session" <> _}} =
               CreatePortalSession.call(customer, subscription, items)
    end
  end
