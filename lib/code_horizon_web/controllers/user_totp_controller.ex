defmodule CodeHorizonWeb.UserTOTPController do
  ...
  def create(conn, %{"user" => user_params}) do
    ...
    case Accounts.validate_user_totp(current_user, user_params["code"]) do
      :valid_totp ->
        CodeHorizon.Logs.log("totp.validate", %{user: current_user})
      ...
      {:valid_backup_code, remaining} ->
        CodeHorizon.Logs.log("totp.validate_with_backup_code", %{user: current_user})
      ...
      :invalid ->
        CodeHorizon.Logs.log("totp.invalid_code_used", %{user: current_user})
