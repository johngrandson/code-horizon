...
config :code_horizon, CodeHorizonWeb.Endpoint,
  ...
  url: [host: "localhost"],
  render_errors: [
    formats: [html: CodeHorizonWeb.ErrorHTML, json: CodeHorizonWeb.ErrorJSON],
    layout: false
  ],
  pubsub_server: CodeHorizon.PubSub,
...
config :code_horizon, CodeHorizon.Mailer, adapter: Swoosh.Adapters.Local
...
config :petal_components,
       ...
       {CodeHorizonWeb.CoreComponents, :translate_error}
...
config :code_horizon, Oban,
  repo: CodeHorizon.Repo,
  queues: [default: 5],
  plugins: [
    {Oban.Plugins.Pruner, max_age: 3600 * 24},
    {Oban.Plugins.Cron,
     crontab: [
       # {"@daily", CodeHorizon.Workers.ExampleWorker}
       # {"* * * * *", EveryMinuteWorker},
       # {"0 * * * *", EveryHourWorker},
...
config :code_horizon, :passwordless_enabled, true
...
config :code_horizon, :content_security_policy, %{
  default_src: [
    "'unsafe-inline'",
    "'unsafe-eval'",
    "'self'",
...
config :flop, repo: CodeHorizon.Repo, default_limit: 20
config :tesla, :adapter, {Tesla.Adapter.Finch, name: CodeHorizon.Finch}

# :user or :org
config :code_horizon, :billing_entity, :org
config :code_horizon, :billing_provider, CodeHorizon.Billing.Providers.Stripe
config :code_horizon, :billing_provider_subscription_link, "https://dashboard.stripe.com/test/subscriptions/"
