defmodule CodeHorizonWeb.PasswordlessAuthLiveTest do
  ...
  describe "no existing user" do
    test "it creates a new user and logs them in", %{conn: conn} do
      inital_user_count = Repo.count(CodeHorizon.Accounts.User)
      ...
      assert inital_user_count + 1 == Repo.count(CodeHorizon.Accounts.User)
      new_user = Repo.last(CodeHorizon.Accounts.User)
      ...
      sign_in_token =
        new_user
        |> CodeHorizon.Accounts.generate_user_session_token()
        |> Base.encode64()

      form = form(view, "form", %{"auth" => %{"sign_in_token" => sign_in_token}})
      conn = follow_trigger_action(form, conn)
      assert redirected_to(conn) =~ CodeHorizonWeb.Helpers.home_path(new_user)
  ...
  describe "with existing user" do
    test "it logs in the existing user", %{conn: conn} do
      ...
      conn = follow_trigger_action(form, conn)
      assert redirected_to(conn) =~ CodeHorizonWeb.Helpers.home_path(user)
      assert inital_user_count == Repo.count(CodeHorizon.Accounts.User)
    end
  end
end
