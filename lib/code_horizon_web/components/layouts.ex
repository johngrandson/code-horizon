defmodule CodeHorizonWeb.Layouts do
  @moduledoc false
  use CodeHorizonWeb, :html

  alias CodeHorizonWeb.LandingPageComponents

  require Logger

  embed_templates "layouts/*"

  def app_name, do: CodeHorizon.config(:app_name)
  ...
  def description(conn) do
    if conn.request_path == "/" || conn.request_path == "/blog" do
      CodeHorizon.config(:seo_description)
    else
      if public_page?(conn.request_path) do
        Logger.warning(
  ...
  def current_page_url(%{request_path: request_path}), do: CodeHorizonWeb.Endpoint.url() <> request_path

  def current_page_url(_conn), do: CodeHorizonWeb.Endpoint.url()
  ...
  def twitter_site(_conn) do
    if CodeHorizon.config(:twitter_url) do
      "@" <> (:twitter_url |> CodeHorizon.config() |> String.split("/") |> List.last())
    else
      ""
    end
  end
  ...
  def public_page?(request_path) do
    ...
    Enum.find(CodeHorizonWeb.Menus.public_menu_items(), &(URI.parse(&1.path).path == stripped_path))
  end
end
