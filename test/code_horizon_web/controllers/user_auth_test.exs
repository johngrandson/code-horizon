defmodule CodeHorizonWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, CodeHorizonWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    ...
    test "redirects to org invites if an invite is pending", %{conn: conn, user: user} do
      org = CodeHorizon.OrgsFixtures.org_fixture()
      CodeHorizon.OrgsFixtures.invitation_fixture(org, %{email: user.email})
      conn = UserAuth.log_in_user(conn, user)
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      CodeHorizonWeb.Endpoint.subscribe(live_socket_id)
