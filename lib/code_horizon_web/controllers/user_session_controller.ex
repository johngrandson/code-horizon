defmodule CodeHorizonWeb.UserSessionController do
  ...
  def create_from_token(conn, %{"auth" => %{"sign_in_token" => sign_in_token} = params}) do
    with true <- CodeHorizon.config(:passwordless_enabled),
  ...
  def redirect_if_passwordless_disabled(conn, _opts) do
    if CodeHorizon.config(:passwordless_enabled) do
      conn
    else
      conn
      |> redirect(to: ~p"/auth/sign-in")
