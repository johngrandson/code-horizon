defmodule CodeHorizon.Accounts do
  @moduledoc """
  ...
  def deliver_user_confirmation_instructions(%User{} = user, confirmation_url_fun)
    ...
    if user.confirmed_at do
      ...
      if CodeHorizon.config(:env) == :dev do
  ...
  def user_lifecycle_action("after_register", user, %{registration_type: registration_type}) do
    ...
    CodeHorizonWeb.AdminDashboardLive.notify_admin_stats()
    CodeHorizon.Orgs.sync_user_invitations(user)

    CodeHorizon.Slack.message("""
    :bust_in_silhouette: *A new user joined!*
    *Name*: #{CodeHorizonWeb.Helpers.user_name(user)}

    #{CodeHorizonWeb.Router.Helpers.admin_user_index_url(CodeHorizonWeb.Endpoint, :edit, user)}
    """)
  end
  ...
  def user_lifecycle_action("after_sign_in", user, %{ip: ip}) do
    ...
    CodeHorizon.MailBluster.sync_user_async(user)
  end
